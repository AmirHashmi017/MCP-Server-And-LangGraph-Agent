"""
LangGraph Agent that uses the unified Python MCP Server
This is the intelligent layer on top of your MCP server
"""
...
class AgentState(TypedDict):
    ...
    volvox_token: str
    project_tokens: dict  # For other project tokens
...
# ============ Tool Definitions ============
# These are LangChain tools that wrap MCP calls
...
def call_model(state: AgentState):
    ...
    # Inject token context if available
    ...
    llm = ChatGoogleGenerativeAI(
        model="gemini-2.5-flash",                    # works perfectly with tools
        temperature=0,
...
def extract_tokens(state: AgentState):
    ...
    # Look for tokens in recent responses
